...
func approvalHandler(approve bool) func(context.Context, Request, *Runtime) error {
	...
	return func(_ context.Context, req Request, rt *Runtime) error {
		if rt == nil || rt.IsAdmin == nil || !rt.IsAdmin() {
			return req.Reply("Permission denied.")
		}
		if rt.ResolveApproval == nil {
			return req.Reply("Approvals unavailable.")
		}
...
func allowCommand() Definition {
	return Definition{
		...
		Handler: func(_ context.Context, req Request, rt *Runtime) error {
			...
			if rt.AllowAdd == nil || rt.AllowRemove == nil || rt.AllowList == nil {
			...
			case "add", "remove":
				...
				number := fields[2]
				if sub == "add" {
					if err := rt.AllowAdd(number); err != nil {
						return req.Reply("Failed to add: " + err.Error())
					}
					return req.Reply("Added " + number + " to the allow list.")
				}
				if err := rt.AllowRemove(number); err != nil {
					return req.Reply("Failed to remove: " + err.Error())
				}
...
func factoryResetCommand() Definition {
	return Definition{
		...
		Handler: func(_ context.Context, req Request, rt *Runtime) error {
			...
			if rt.FactoryReset == nil {
				return req.Reply("Factory reset unavailable.")
			}
